...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv, Env, FBinding, getFBindingVal, getFBindingVar } from "./L4-env-box";
...
import { Result, bind, mapv, mapResult, makeFailure, makeOk, isOk, safe2 } from "../shared/result";
...
const evalTraceExp = (exp: TraceExp): Result<void> => 
    mapv(applyEnvBdg(theGlobalEnv, exp.var.var), (fbinding: FBinding) =>
        isClosure(getFBindingVal(fbinding)) ? setFBinding(fbinding, makeTracedClosure(getFBindingVal(fbinding) as Closure, getFBindingVar(fbinding))) : 
                                            setFBinding(fbinding, getFBindingVar(fbinding)));
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    // goal: calculate and return the value from the traced closure.
    // 1. call printPreTrace on the name, params and counter
    // 2. evalSequence
    // 3. call printPostTrace with the val of evalSequence and counter going down
    // 4. return the value of evalSequence
    const vars = map((v: VarDecl) => v.var, proc.params); // get the vars from the closure.
    printPreTrace(proc.closureName, args, proc.counter);
    proc.counter = proc.counter + 1;
    const evalSeq = evalSequence(proc.body, makeExtEnv(vars, args, proc.env));
    proc.counter = proc.counter - 1;
    isOk(evalSeq) ? printPostTrace(evalSeq.value, proc.counter) : evalSeq;
    return evalSeq;
}
