...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) => 
        ...
        parseGoodTraceExp(first(params));

export const parseGoodTraceExp = (vref: Sexp): Result<TraceExp> =>
    ! isIdentifier(vref) ? makeFailure("The arg should be an identifier") :
    mapv(parseL4CExp(vref), (x: CExp) => makeTraceExp(makeVarRef(vref)));
